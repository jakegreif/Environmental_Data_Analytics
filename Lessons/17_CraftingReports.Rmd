...
#This is the header at the beginning of each rmd
2. **R Code Chunks** surrounded by ``` on top and bottom
    + Create using `Cmd` + `opt` + `I`
...
A handy cheat sheet for R markdown can be found [here](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf).
Another one can be found [here](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf).

## WHY R MARKDOWN?

<Fill in our discussion below with bullet points. Use italics and bold for emphasis>
<Arrows on either side of a sentence omit it from the rmd doc>
The following symbols can be used to make bullet point list (-,+,*). All need a space afterwards

- Readable, legible code

- Code output, text, and notes are all in one document

- Can be knit to useful formats (PDF, html, docx) 
...
Dataset     |  Details
----------- |  -------
EPA Air     |  Ozone and PM 2.5 data in NC for 2017 and 2018
Lakes       |  Nutrient and physical data for Peter and Paul lakes
Stream Flow |  Stream flow on the Eno River at Hillsborough

<dashed lines determine the width of your boxes. If you have a tall, skinny table, you can fit it to the page better with more dashes>
...
```{r include=FALSE}
Lake_nut_raw <- read_csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")

head(Lake_nut_raw)

Lake_nut_raw$sampledate <- as.Date(Lake_nut_raw$sampledate, format = "%m/%d/%Y")
class(Lake_nut_raw$sampledate)
```
...
```{r}
Lake_nut_skinny <- select(Lake_nut_raw, -lakeid, -depth_id, -comments)

Lake_nut_surface <- Lake_nut_skinny %>%
  filter(depth == 0)
```
...
```{r include=FALSE}
Lake_N_summary <- Lake_nut_surface %>%
  group_by(lakename) %>%
  summarise(meanN = mean(tn_ug, na.rm = TRUE),
            minN = min(tn_ug, na.rm = TRUE),
            maxN = max(tn_ug, na.rm = TRUE),
            stdN = sd(tn_ug, na.rm = TRUE))

Lake_P_summary <- Lake_nut_surface %>%
  group_by(lakename) %>%
  summarise(meanP = mean(tp_ug, na.rm = TRUE),
            minP = min(tp_ug, na.rm = TRUE),
            maxP = max(tp_ug, na.rm = TRUE),
            stdP = sd(tp_ug, na.rm = TRUE))
```
...
```{r echo=FALSE}
kable(Lake_N_summary)
kable(Lake_P_summary)
```


Create a fourth and fifth R chunk that generates two plots (one in each chunk): one for total N over time with different colors for each lake, and one with the same setup but for total P. Decide which geom option will be appropriate for your purpose, and select a color palette that is visually pleasing and accessible. Customize the chunk options such that the final figures are displayed but not the code used to generate the figures. Lastly, add a fig.cap chunk option to add a caption (title) to your plot that will display underneath the figure.
```{r echo=FALSE}
ggplot(Lake_nut_surface) +
