...
PeterPaul.summertemp <- 
  PeterPaul.chem %>%
  #select columns lakename through temp
  ...
  #add a "week" column to represent seasonality (week() function is from lubridate)
  mutate(Week = week(sampledate)) %>%
  #autocorrelation code (further down) won't work if there are NAs
  na.exclude()
...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     ...
                     random = ~1|Week)
# take dataset we just made
# (Fixed effects model w/ interaction term) predict temp based on sample date and lake
# specify that our random variable is week
TempTest.auto
# Output gives us the variability of temperature (1.5 degrees) across weeks

ACF(TempTest.auto)
# This tells us that 42% of the variability in our data is explained by the variable of weeks
```
...
TempTest.mixed <- lme(data = PeterPaul.summertemp,
                     ...
                     #time variable|random effect
                     #define method as restricted maximum likelihood
                     method = "REML")
summary(TempTest.mixed)
# Output: First line tells us that the temp. in week 1 at Paul Lake is 20.47 degrees
# Second line tells us that temp in Paul Lake increases by 0.0001 degree each week
...
# fixed effect may be better to use if you have a small sample size or 

anova(TempTest.mixed, TempTest.fixed)
# The lower the AIC, the better.
# The p-value tells us that those models have a significantly different fit
