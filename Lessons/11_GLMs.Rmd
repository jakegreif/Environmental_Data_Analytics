...
# removing negatives and set depth_id to factor is just prepping the data to answer a question later on
...
# This simply tests the assumption that our data approximate a normal distribution. We know that our assumption is wrong because our p value is super low

ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram(stat = "count")
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)
# This shows us graphically how well our data follows the normal distribution. The closer our points are to the line, the more it follows the normal distribution

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample
# This runs a t test, setting our alternative hypothesis as O3 is less than 50
```

What information does the output give us? How might we report this information in a report?

> ANWSER: It gives us our t value, and we might report our t statistic, degrees of freedom, and our confidence interval. We're 95% confident that the average is less than 50 (Proper reporting: Ozone AQI values in 2017-18 were significantly lower than 50 (one sample t-test; t = -41.9, df = 184, p < 0.0001)) 
...
# The output here is a bit confusing, but the second row of coefficients tells us the stats for Bryson, and the first row is stats for Blackstone. Under estimate, Bryson is 3.3 less than Blackstone
```
...
O3.twosample.wilcox
# This gives you less information than a t-test. If the data is really not following the normal, then this might be better to use (b/c it's non-parametric)
```
...
# Use this if were testing more than 2 populations. It tells us about the variance between populations
...
# Run this if you use aov instead of lm. This shows us stats for all of the permutations relating one population to another
...
> ANSWER: We could indicate which sites are significantly different by placing a star next to the significant sites on the graph. REPORT: (ANOVA; F = 16.16, df = 1898, p < 0.0001)
...
# This is combining the 2 variables and comparing the combinations
