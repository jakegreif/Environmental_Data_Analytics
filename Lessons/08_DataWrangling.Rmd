...
# This gives us the comments about Peter Paul lake made when collecting data
...
# drop.levels is a function that will remove information stored from the initial dataset from your filtered dataset. The command above shows us that comments were stored from other lakes that we don't need
...
# We don't want R to save a count column, therefore row.names = FALSE.
...
# We're specifying which columns we want to keep. If we wanted to specify which columns to remove, just place a minus sign before each column name
```
...
# This takes the last 5 columns (tn_ug through po4) and places the column names in the 'nutrient' column, and the values in the 'concentration' column
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
# This is subsetting the dataset on the condition (!) that NA is not in the concentration column
...
# Opposite of gather, this is spreading the nutrient and concentration columns, meaning it takes the elements from the nutrient column and makes a column for each element, and then populates those columns with the associated concentrations
...
# This separates the date into 3 columns, and it does NOT keep the original column
```
...
# Adds all columns and rows only matching columns from one dataset to another dataset, which one comes first makes a difference
...
library(lubridate)
# The message that appears with this is just telling us that 
...
# The second line above is useful when your dataset is changing (continuing to grow). first and last functions should be in lubridate package
```
...
NTL.PeterPaul.summaries <- 
  ...
  # Grab our dataset
  filter(depth == 0) %>%
  # Show only rows at depth 0
  group_by(lakename) %>%
  # Group by lake name
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>%
  # Remove rows that have NA for all of these (i.e. keep rows that have all 3 columns filled)
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
# Give mean and std for each column
# Pipe, or %>%, means 'and then...'
