...
print(O3plot)
# Example of default ggplot theme

O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray()
print(O3plot1)
# The default is also called theme_gray, so the plot here is the same as the default above

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2)
# theme_bw changes background to white, with a gray grid, and with a black border around it

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3)
# theme_light just lightens the dark lines from theme_bw

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()
print(O3plot4)
# theme_classic has a blank background
```
...
# legend.position can be top, right, left, bottom. But we can also use position and justification commands to place the legend wherever we want it.
# To change things within a theme, reference line 73
...
theme_set(mytheme)
# This is telling R to set the default theme as mytheme. Now every ggplot we make in this document will have mytheme settings
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  # This code sets the fill color from 2016-2020 on the x axis and 0-50 on the y axis. If fill was set to Jan 2017 to Jan 2019, it wouldn't fill the whole graph. 
  # geom_rect fills the background
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") +
  geom_point() +
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +
  # Sets the label for the green section as good at y=45 and hjust moves the word 1 unit to the left (hjust is horizontal justification)
  ...
  # scale_x_date sets gthe 
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) +
  # brackets place the 3 in O3 as a subscript, and ^ is for superscript
...
# viridis color palet hex codes always end with ff
...
NvsP2 <-
  ...
  #xlab(expression("Total P (\U003bCg/L)")) +
  #ylab(expression("Total N (\U003bCg/L)"))
  labs(x = "Total P (\U003bCg/L)", y = "Total N (\U003bCg/L)", color = "Depth, m", shape = "Lake") +
...
# place your additional edits here
  labs(y = "Concentration, \U003bCg/l", x = "", color = "Nutrient") +
...
# This allows you to put multiple plots on one page
```
