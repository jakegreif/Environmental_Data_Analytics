...
# cord_equal() scales the axes a bit better
```
...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  addTiles() %>% 
  # tiles are the background maps, here we're using the default
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   radius=(~meanPM*2),
                   stroke = FALSE, 
  # stroke is the outline of the features
                   fillOpacity = 0.3,
                   popup = ~as.character(`Site.Name`))
  # popup makes labels appear when you click on a feature, labeled by Site.Name
#Show the map
myMap
```
...
# geometry is a new column, and it consolidated the latlong columns

#What is the class of the values in the geometry column?
class(EPAair_PM_avg_sf$geometry)
# "sfc_POINT" which us a point features (sfc means simple feature column)

#What does this look like
head(EPAair_PM_avg_sf)
# Gives us (x,y) looking coordinates
...
# geom_sf plots our geometry as a spatial feature (axes are coordinates)
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1)
# st_buffer uses units of your geometry (in this case it's degrees)
...
# st_crs is used to ID the coordinate system
# EPSG refers to a specific coordinate system that we can look up using the link below. 
# proj4string gives us the projection and datum
```
...
# convert to UTM 17N using the crs code, which can be found using spatialreference.org
...
# StateFP is just a code for each state in the country
...
st_bbox(counties_sf)
#bbox is short for bounding box if we want to put a simple box around our data to approximate the extent
...
# Summarize function is summing the area of the counties in NC
...
# st_union is a function that combines the geomtries of a dataset. It creates a list of geometries
mapview(state_sf2)
```
...
huc2_sf <- huc8_sf %>%
 ...
 # This took our HUC dataset, and grouped rows based on SUB, which is the HUC2 code, and then summed the areas of the HUC8s within each HUC2, creating a layer of HUC2s. 
...
# st_transform just converts CRS to another CRS
...
# leaflet can ONLY use WGS84.
# WGS84 is best for displaying maps, but UTM is best for spatial analysis because units are in meters, not degrees.
```
...
ggplot() +
  ...
  scale_fill_gradient(low="yellow", high="darkgreen")
# same code, but the ordering matters
```
...
leaflet() %>% 
# create leaflit object
  addProviderTiles(providers$Esri.WorldShadedRelief) %>%  
# Adds a specific base map, which can be found at the link above 
...
# addMarkers adds a little pin that can include pop up info
...
latticeview(m1, m2)
sync(m1,m2)
# syncs up maps so when you move on one, the action is mimiced on the other one
```
...
# st_intersection(layer to be clipped, layer that defines clip boundary)
mapview(huc2_UTM_nc)
```
...
huc2_0302_utm <- huc8s_sf_utm %>%
  filter(SUB == '0302')

huc0302_counties <- st_intersection(counties_sf_utm,huc2_0302_utm)
...
## Attribute joins with `merge`
To add more attributes to our spatial features, we can merge records just as we do with non-spatial dataframes, i.e. with the `merge` command. Here we will read in a table of county census data and join them to our counties_sf data set.
...
# by is specifying that these columns will be the reference columns for the join
# if we joined our counties dataset to our demography dataset, then it would not see the joined dataset as a spatial dataframe anymore.
...
# We're creating a boolean mask to see where our point resides

#Filter the counties dataset using the boolean mask
selCounties <- counties_sf_utm[resultMask,]
# Filter it to isolate the county our point is in
...
# Create a buffer around our point, and then repeat the process above to see which counties it intersects
