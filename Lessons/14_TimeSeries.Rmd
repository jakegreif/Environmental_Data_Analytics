...
PeterPaul.nutrients.surface <- 
  PeterPaul.nutrients %>%
  select(-lakeid, -depth_id, -comments) %>%   #remove these (indicated by -)
  filter(depth == 0) %>%
  filter(!is.na(tp_ug))   #filter na's in total phosphorus
...
# z tells us the direction (+/-) and magnitude of our trend. Output here (4.4) tells us that there's a somewhat strong positive trend
```
...
# If p > 0.05, there is no change point 
# Tells us the relative position in the dataset where the change occurs (output tells us row 35)
...
# Output tells us change point is at position 79, so in our full dataset the second change point is in row 35+79 (=114)
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  scale_color_manual(values = c("#7fcdbb", "#253494")) +
  geom_vline(xintercept = as.Date("1993/05/20"), color = "#253494", lty = 5) +
  geom_vline(xintercept = as.Date("1998/01/01"), color = "#253494", lty = 5)
# Notice that we have to tell ggplot to read the x values as date again
# sometimes we need to supply the origin date, placing origin = "1970/01/01" after the as.Date part of the function
```
...
ggplot(USGS.flow.data.trimmed, aes(x = datetime, y = discharge.mean)) +
  ...
  scale_y_log10()
# adjusting the y scale makes the data easier to visualize without adjusting our data
```
...
> ANSWER: spline works a bit better when you know the mathematics behind your data vs linear. In environmental data from natural systems we don't really know the exact math behind it, so its usually best to use linear
...
# seasonal man kendall can only use time series, so use the ts() function to make the dataframe a timeseries, starting at Jan. 2009, and setting the frequency to 12 (monthly)

# Run a Seasonal Mann-Kendall test
USGS.smktest <- smk.test(USGS.flow.data.timeseries)
USGS.smktest
summary(USGS.smktest)
# summary gives us a list of trends by month
```
