...
read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") # It uploads all the data into your console, and it cuts off your data because it's too large for R to handle

# Another option is to choose with your browser
# read.csv(file.choose()) # It isn't so efficient

# To import .txt files, use read.table rather than read.csv
#read.table()


# Relative file path is definitely the easiest way
```
...
str(USGS.flow.data) # Tells us about the strucutre of the data, including info about data types by row
dim(USGS.flow.data)
length(USGS.flow.data)

head(USGS.flow.data) # Gives us first 6 rows of each column, this is default
head(USGS.flow.data, 10) # Specifies how many rows of each column you want to see
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] # Gives us rows 30000-30005 and columns 3, and 14

class(USGS.flow.data$datetime) # What type of data is the date and time column?
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) # Gives you basic stats about numerical data
...
> ANSWER: They were populated with NA if the column is numeric
