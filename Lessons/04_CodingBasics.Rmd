...
vector1[3] # Provides the 3rd element of vector1, called "call out"

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # numeric matrix, ordered by column (by default)
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # numeric matrix, ordered by row
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) # Names column and then row
matrix3

matrix1[4, ] # Shows elements from row 4
matrix1[ , 3] # Shows elements from column 3
matrix1[c(12, 14)] # Show elements 12 and 14 
matrix1[c(12:14)] # Show elements 12 through 14
matrix1[2:4, 1:3] # Show elements in row 2 through 4, and column 1 through 3

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # Creates matrix with specified names and dimensions by row
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] # Shows the second component of list

# Data Frames ----
d <- c(1, 2, 3, 4) # numeric vector
e <- c("red", "white", "red", NA) # character vector
f <- c(TRUE, TRUE, TRUE, FALSE) # logical vector
dataframe1 <- data.frame(d,e,f) # Creates a data frame (i.e. table) with 3 parameters
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # Opens window of dataframe table

dataframe1[1:2] # Show columns 1 and 2
dataframe1[1:2,] # Show rows 1 and 2
dataframe1[c("ID","Passed")] # Show columns titled ID and Passed
dataframe1$Color # Show elements from data labled 'Color'
dataframe1$

```
QUESTION: How do the different types of data appear in the Environment tab?

> ANSWER: Either as data or values. Data is the entire data frame, and values are elements we have assigned a value to.

QUESTION: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> ANSWER: A drop down menu appears that allows me to select a category (column) that has been assigned

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER: It makes the section collapsable
...
} # Your ingredients go in the parenthesis, and your steps go inside curly brackets
```
